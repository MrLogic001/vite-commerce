...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      let orderItems = []  
      const prodsData = structuredClone(products) 

      for (const prodsId in cartItems) {
        for (const sizes in cartItems[prodsId]) {
          if (cartItems[prodsId][sizes] > 0) {
            const itemInfo = prodsData.find(product => product._id === prodsId)
            
            if (itemInfo) {
              itemInfo.size = sizes
              itemInfo.quantity = cartItems[prodsId][sizes]
    ...
    >
      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-[450px]">
        <div className="text-xl sm:text-2xl my-3">
          <Title text1={"DELIVERY"} text2={"INFORMATIO"} />
        </div>
